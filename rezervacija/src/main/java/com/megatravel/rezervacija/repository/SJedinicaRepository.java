...
public interface SJedinicaRepository extends JpaRepository<SJedinica, Long> {
    ...
    @Query("SELECT DISTINCT sj FROM SJedinica sj LEFT OUTER JOIN sj.rezervacije r WHERE sj.id=?3 AND ((r.od >=?1 AND r.od <= ?2) OR (r._do >=?1 AND r._do <= ?2) OR (?1 >=r.od AND ?1 <= r._do) OR (?2 >=r.od AND ?2 <= r._do))")
